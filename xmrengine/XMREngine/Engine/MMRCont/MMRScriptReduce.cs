...
namespace MMR {
	...
	public class ScriptReduce {
		...
		private static Dictionary<Type, int> precedence = null;

		/**
		 * @brief initialize operator precedence table
		 * @returns with 'precedence' initialized
		 */
		private static void PrecedenceInit ()
		{
			if (precedence == null) {
		 ...
		 *        Performs little semantic testing, ie, undefined variables, etc.
		...
		public TokenRVal ParseRVal (ref Token token, Type termTokenType)
		{
			...
			while (token.GetType () != termTokenType) {
				...
				/*
				 * If there are stacked operators of higher precedence than new one,
		...
		private BinOp GetOperator (ref Token token)
		{
			if (precedence.ContainsKey (token.GetType ())) {
				...
				binOp.token  = token;
				binOp.preced = precedence[token.GetType()];
				token = token.nextToken;
				return binOp;
			}
